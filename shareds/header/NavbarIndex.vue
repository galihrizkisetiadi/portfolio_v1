...
<template>
    ...
    >
        <UButton
            color="gray"
            variant="ghost"
            class="mr-auto h-[40px] focus:border-0"
        ...
        >
            <Transition enter-from-class="translate-y-[150%] opacity-0" enter-active-class="transition duration-300">
                <div v-if="colorMode.value === 'dark'" class="group flex flex-col">
                    <Icon name="material-symbols:dark-mode-outline" class="text-[#FDBA74]" />
                </div>
                <div v-else class="group flex flex-col">
                    <Icon name="material-symbols:light-mode-outline" class="text-white" />
                </div>
            </Transition>
        </UButton>
